...
export default function Sorteo() {
  ...
  const confirmarCompra = async () => {
    ...
    setCargando(true)
    try {
      const { data } = await axios.post(`${API}/sorteos/comprar`, { numeros: seleccionados }, { headers })
      ...
      cargarMisBoletas()
      alert('✅ ¡Boleta comprada! Recibiste un bono de $500.')
    } catch (err) {
      alert(err.response?.data?.error || 'Error al comprar boleta')
...
Código: ${usuario?.codigo_referido}
...
3 primeras (123X) → $80.000
3 últimas (X234) → $80.000
...
¡Mucha suerte!
  ...
  const solicitarRecarga = async () => {
    ...
    try {
      ...
      setMensajeRecarga('❌ ' + (err.response?.data?.error || 'Error al enviar solicitud'))
    } finally { setEnviando(false) }
  }

  const solicitarRetiro = async () => {
    if (!retiroMetodo) return alert('Selecciona un método de retiro')
    ...
    try {
      ...
      setMensajeRetiro('✅ Solicitud enviada. El admin procesará tu retiro pronto.')
      setSaldo(prev => prev - Number(retiroMonto))
      setRetiroMonto(''); setRetiroMetodo(''); setRetiroDatos('')
      cargarMisRetiros()
      setTimeout(() => setMensajeRetiro(''), 5000)
    } catch (err) {
      setMensajeRetiro('❌ ' + (err.response?.data?.error || 'Error al solicitar retiro'))
  ...
  const estadoRetiroColor = (estado) => {
    ...
    <div style={s.container}>
      ...
      <div style={s.header}>
        <div>
          ...
          <div style={s.sub}>Hola, {usuario?.nombre} · <span style={s.gold}>{usuario?.codigo_referido}</span></div>
        </div>
          ...
          {mostrarBoleta && (
            <div style={s.boletaCard}>
              <div style={s.boletaHeader}>
                <div style={{ fontWeight: '600' }}>🎟️ Tu boleta · Sorteo #{String(sorteo?.id || 1).padStart(4,'0')}</div>
                <div style={{ fontSize: '12px', color: '#888' }}>$5.000 COP · Bono: +$500</div>
              </div>
      ...
      {tab === 'premios' && (
        <div>
          {[
            ...
            { color: '#9333ea', titulo: '🥈 Tres primeras cifras', desc: '123X · hasta 9 ganadores', valor: '$80.000 c/u' },
            { color: '#3b82f6', titulo: '🥉 Tres últimas cifras', desc: 'X234 · hasta 9 ganadores', valor: '$80.000 c/u' },
            { color: '#22c55e', titulo: '🎁 Dos últimas cifras', desc: 'XX34 · hasta 81 ganadores', valor: 'Boleta gratis' },
            { color: '#f97316', titulo: '🎁 Bono por compra', desc: 'Recibe $500 de bono por cada boleta que compres', valor: '$500 c/u' },
          ].map((p, i) => (
      ...
      {tab === 'billetera' && (
        <div>
          ...
          <div style={s.card}>
            ...
            {metodo && (
              <div style={{ background: '#111', borderRadius: '8px', padding: '12px', marginBottom: '12px', fontSize: '13px' }}>
                <div style={{ color: '#888', marginBottom: '4px' }}>Datos de pago:</div>
                <div style={{ color: '#D4AF37', fontWeight: '600' }}>{metodos.find(m => m.id === metodo)?.info}</div>
              </div>
            )}
            <div style={s.field}>
              <label style={s.label}>Monto (mín. $5.000)</label>
              <input style={s.input} type="number" placeholder="Ej: 10000" value={monto} onChange={e => setMonto(e.target.value)} />
            </div>
            <div style={s.field}>
              <label style={s.label}>Número de comprobante</label>
              <input style={s.input} placeholder="Ej: 123456789" value={comprobante} onChange={e => setComprobante(e.target.value)} />
            </div>
            ...
            <button style={{ ...s.btnPrimary, width: '100%', opacity: enviando ? 0.7 : 1 }} onClick={solicitarRecarga} disabled={enviando}>
              {enviando ? 'Enviando...' : 'Solicitar recarga'}
            </button>
          </div>
          ...
          <div style={s.card}>
            ...
            <div style={{ fontSize: '12px', color: '#888', marginBottom: '12px' }}>Retiro mínimo: $10.000 · El admin procesará tu solicitud manualmente</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '14px' }}>
              {metodos.map(m => (
            ...
            <div style={s.field}>
              <label style={s.label}>Monto a retirar (mín. $10.000)</label>
              <input style={s.input} type="number" placeholder="Ej: 50000" value={retiroMonto} onChange={e => setRetiroMonto(e.target.value)} />
            </div>
            <div style={s.field}>
              <label style={s.label}>Número de cuenta / dirección de wallet</label>
              <input style={s.input} placeholder="Ej: 3001234567 o TXabc..." value={retiroDatos} onChange={e => setRetiroDatos(e.target.value)} />
            </div>
            {mensajeRetiro && <div style={{ fontSize: '13px', color: mensajeRetiro.startsWith('✅') ? '#4ade80' : '#f87171', marginBottom: '10px' }}>{mensajeRetiro}</div>}
            <button style={{ ...s.btnPrimary, width: '100%', opacity: enviandoRetiro ? 0.7 : 1 }} onClick={solicitarRetiro} disabled={enviandoRetiro}>
              {enviandoRetiro ? 'Enviando...' : 'Solicitar retiro'}
            </button>
              ...
              <div style={{ marginTop: '16px' }}>
                ...
                {misRetiros.map(r => (
                  <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #2a2a2a', fontSize: '12px' }}>
                    <div>
                      <div>${r.monto.toLocaleString('es-CO')} · {r.metodo}</div>
                      <div style={{ color: '#666', marginTop: '2px' }}>{new Date(r.created_at).toLocaleDateString('es-CO')}</div>
                    </div>
                    <span style={{ color: estadoRetiroColor(r.estado), fontWeight: '600' }}>{r.estado}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={s.card}>
            ...
            {movimientos.length === 0 ? (
              <div style={s.empty}>No hay movimientos aún</div>
            ) : movimientos.map(m => (
              <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #2a2a2a', fontSize: '13px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: '#666' }}>{new Date(m.created_at).toLocaleDateString('es-CO')}</div>
                </div>
                <div style={{ fontWeight: '600', color: m.monto > 0 ? '#4ade80' : '#f87171' }}>
      ...
      {tab === 'misboletas' && (
        <div>
          {misBoletas.length === 0 ? (
            <div style={s.empty}>No tienes boletas aún. ¡Compra tu primera boleta!</div>
          ) : misBoletas.map(b => (
            <div key={b.id} style={s.boletaCard}>
              <div style={s.boletaHeader}>
                <div style={{ fontWeight: '600' }}>Boleta #{String(b.id).padStart(3,'0')} · {b.sorteos?.nombre}</div>
              ...
              <div style={s.numerosGrid}>
                {b.numeros?.map(n => <div key={n} style={s.numBoleta}>{n}</div>)}
              </div>
              {b.sorteos?.numero_ganador && (
                <div style={{ fontSize: '12px', color: '#D4AF37', marginBottom: '10px' }}>
                  Número ganador: <strong>{b.sorteos.numero_ganador}</strong>
                </div>
              )}
              <button style={{ ...s.btnSecondary, fontSize: '12px', padding: '7px 14px' }} onClick={() => descargarBoleta(b)}>
                ⬇ Descargar boleta
              </button>
            </div>
          ))}
...
const s = {
  ...
  tabs: { display: 'flex', gap: '4px', marginBottom: '1.2rem', borderBottom: '1px solid #2a2a2a' },
  tab: { padding: '8px 14px', fontSize: '13px', background: 'none', border: 'none', cursor: 'pointer', color: '#666', borderBottom: '2px solid transparent', marginBottom: '-1px' },
  ...
  modeBtn: { fontSize: '12px', padding: '6px 10px', background: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', cursor: 'pointer', color: '#888' },
